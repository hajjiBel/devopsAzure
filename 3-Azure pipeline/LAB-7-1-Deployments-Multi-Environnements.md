...
### Étape 5: Créer une Procédure de Rollback
1. Créez le fichier **docs/runbook-rollback.md**:

```markdown
# Runbook: Procédure de Rollback

## Scénario 1: Rollback après Blue-Green
**Temps d'exécution**: ~2 minutes

### Étapes
1. Vérifier les logs d'erreur dans Application Insights
2. Si erreurs critiques détectées:
   - Basculer le trafic du VERT vers le BLEU
   - Valider que BLEU fonctionne correctement
   - Notifier l'équipe
3. Investigation du problème
4. Fix et nouveau déploiement

## Scénario 2: Rollback partiel (Canary)
**Temps d'exécution**: ~1 minute

### Étapes
1. Vérifier taux d'erreur Canary
2. Si taux > seuil accepté:
   - Arrêter le déploiement Canary
   - Reconduire 100% du trafic à la version stable
   - Investiguer les problèmes
3. Créer incident pour investigation

## Contacts d'Escalade
- **Tier 1**: DevOps Team
- **Tier 2**: Architecture Team
...
1. Commitez toutes les modifications:
   ```bash
   mkdir -p templates/strategies
   git add .
   git commit -m "Ajout des stratégies Blue-Green et Canary, runbook de rollback"
   git push origin main
   ```

2. Testez le pipeline:
   - Lancez un déploiement en Development
   - Observez les étapes de build, test, et déploiement
   - Vérifiez que les health checks passent

**Temps estimé**: 10 minutes

### Étape 7: Implémenter les Métriques de Santé
1. Créez un template de monitoring **templates/monitoring/health-check.yml**:
...
steps:
- script: |
    echo "Vérification de santé: ${{ parameters.healthCheckUrl }}"
    
    for i in {1..${{ parameters.maxRetries }}}
    do
      echo "Tentative $i..."
      
      RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" \
        "${{ parameters.healthCheckUrl }}")
      
      if [ $RESPONSE -eq 200 ]; then
        echo "✓ Endpoint accessible (200 OK)"
        exit 0
      fi
      
      if [ $i -lt ${{ parameters.maxRetries }} ]; then
        echo "En attente de ${{ parameters.retryDelaySeconds }} secondes..."
        sleep ${{ parameters.retryDelaySeconds }}
      fi
    done
    
    echo "✗ Health check échoué après ${{ parameters.maxRetries }} tentatives"
    exit 1
  displayName: 'Health Check avec Retry'
```
